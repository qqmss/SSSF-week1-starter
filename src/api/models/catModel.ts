...
const updateCat = async (
  ...
  const [rows] = await promisePool.execute<RowDataPacket[]>(
    `
    SELECT owner FROM sssf_cat WHERE cat_id = ?;
    `,
    [catId]
  );
  if (rows.length === 0 && role === 'user') {
    throw new CustomError('No cat owner found', 404);
  }
  const catOwnerId = rows[0].owner;
  if (role === 'user' && catOwnerId !== userId) {
    throw new CustomError('Unauthorized', 401);
  }
  /*
  const [headers] = await promisePool.execute<ResultSetHeader>(
    `
    UPDATE sssf_cat SET cat_name = ?, weight = ?, filename = ?, birthdate = ?, coords = POINT(?, ?)
    WHERE cat_id = ?;
    `,
    [
      data.cat_name,
  ...
  */
  ...
  if (headers.affectedRows === 0) {
    throw new CustomError('No cat updated', 304);
  }
  return true;
};
